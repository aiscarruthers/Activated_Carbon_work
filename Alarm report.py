...
#### is altering the clean data frame globally and actually is required by the time check function cleanDf.index=cleanDf.DTime
#### need to change the longest ststs function so it looks at the first value of alarm status to decide if it is time in alarm or between maybe need to add a cumulative time to get total time in alarm


#### need to do a long term alarm frequency study
...
#updated to fix issue with graphs having bottom axis cut off when saving

os.chdir('C:\\Users\\phester\\PyWork\\Alarms')  # Would need to change this to run elsewhere


def cleanFile(fileName):

    
    file = open(fileName,encoding='ISO-8859-1') # Need this encoding to read
    
    text = file.readlines() # Reads file into text
    ...
    for line in text:
        if 'Fix32' in line:
            fix32.append(line) # This copies operator fix32 actions into a seprate list 
            
        elif 'acknowledged' in line: # Copies operator acknowledgements into a list could perhaps throw these witht he fix32s
            ack.append(line)
            
        elif 'NORITSV1::OPERATOR' in line: # Copies operator acknowledgements into a list could perhaps throw these witht he fix32s
            other.append(line)    
            
        elif '[NORIT   ]' in line: # After the first two filters anything witht his is the data i want
            clean.append(line)
            
        else:
            other.append(line) # catches the unspecified shit     
    ...
    df= pd.read_fwf('cleantext.txt',header=None,parse_dates=[[0,1]],dayfirst=True)
    df = df.drop([2,3],axis=1)
    
    if len(df.columns)>6: # i have added this becuase for some reason some of the dfs can have an extra colum that breaks the next bit not an ideal solution though
        df = df.drop([7],axis=1)
    ...
    if (df['Desc'].isna().values.any()) and (not df['Units'].isna().values.any()): #sometimes there are no units so desc ends up in the wrong column
        df['Desc']=df['Units']
    
    return df

#df = cleanFile('180513.txt') #needs to get brought into the full run function


# This block grabs the top ten by count saves graph and alsoreturns list of tags
def topTen(cleanDf):
    
    notOkDf = cleanDf.query('Status != "OK"').iloc[:,0:2].groupby(['Tag'],as_index=False).count() # this groups all the identical tag counst together using the queery which also gives unique tag list
...
#tagsTT = topTen(df) # another bit that needs to get bundkled up into the master function

# this block here is getting the days alarm profile and sving it as a figure doesnt return anything
def dailyT(cleanDf): # could change this in future so it accepts a resample frequency argument

    cleanDf.index=cleanDf.DTime # this step is required for other blocks and actually alters the global df
    
    resampleCount = cleanDf.query('Status != "OK"').resample('H').count() # numberT gives intervals of that many mins can change for H to get hours
...
#dailyT(df) # another bit for the main function to goble up
...
#longTags = longestStats(df) # gobble this up into the main function
...
def descList(listTags,cleanDf): #Insert a list of tags + dataframe and it will give you a dcitionary of tag:desc
    ...
    listyboi=[]
    for t in listTags:
        tgs[t] = list(cleanDf.query('Tag == @t')['Desc'].unique()) # query for tag equal to the one in the for loop, get the unique descriptions and put into dic
        
        listyboi.append(str(t)+' '+str(tgs[t])[2:-2])
    
        
        # so far has only retuned indivdual description but should put out a list if there is more than one descrip, not sure if this will break something else
    return listyboi

#tagLists = [descList(longTags[0],df),descList(longTags[1],df),descList(tagsTT,df)] # gobble this up intot he main function
...
def sendMail(html,fp='C:\\Users\\phester\\PyWork\\Alarms'):
    ...
    #Msg.To = 'peter.hester@cabotcorp.com'
    Msg.To = 'peter.hester@cabotcorp.com;Arran.Gibson@cabotcorp.com;Mairi.NicolWood@cabotcorp.com;Alan.Thompson@cabotcorp.com'
...
def mainLoop(fileName):    
    ...
    tagLists = [descList(longTags[0],df),descList(longTags[1],df),descList(tagsTT,df)] # Get a list of dictionaries the tags are the keys and descriptions to go with the respective graphs
...
def findFiles(filePath='C:\\Users\\phester\\PyWork\\Alarms\\Bulk'):
    os.chdir(filePath)
    ...
    for file in files:
        ...
        if ('cleantext.txt' in file) or ('.png' in file):
            continue
            
        print(file)
        cleanDf = cleanFile(file)
        
        dfs.append(cleanDf) # thios doesnt work properly as i need to add a date back in against them
...
def week():    
    ### wont attach the images as they are now refrencing the worng place could pu in a os.fucntion to find current working folder
