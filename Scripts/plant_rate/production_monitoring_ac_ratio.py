...
class Production_day:
    ...
    def set_moisture(self):
        self.moisture = float(input("What is the OSF moisture percentage for {}?: ".format(self.date.strftime("%Y-%m-%d"))))
    
    def get_production_periods(self):
        periods = []
        for i in self.up_times:
            Prod = Production_period(self.date, i[0], i[1], self.moisture)
            periods.append(Prod)
...
class Production_run:
    ...
    def V_901_ctrl_chart(self):
        ...
        def plot_IMR(df, tag, SP):
            ...
            if isinstance(SP, float) and tag.split("_")[0] != "ratio":
...
for i in range(1,12):
    D = Production_day(date=dt.datetime(year=2023, month=6, day=19)-dt.timedelta(days=i))
    D.ELCD_control()
